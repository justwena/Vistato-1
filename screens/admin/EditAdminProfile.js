...
const EditAdminProfile = () => {
  ...
  const fetchUserData = async () => {
    ...
    if (user) {
      try {
        const userSnapshot = await firebase.database().ref(`admins/${user.uid}`).once('value');
    ...
    if (user) {
      try {
        ...
        if (profilePictureUri !== originalProfilePictureUri) {
          const downloadURL = await uploadProfilePicture(profilePictureUri, user);

          await firebase.database().ref(`admins/${user.uid}`).update({
  ...
  const checkIsChanged = () => {
    return (
      ...
      (profilePictureUri !== '' && profilePictureUri !== originalProfilePictureUri) ||
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        >
          {isLoading ? (
            <ActivityIndicator size="small" color="#088b9c" style={{ marginRight: 12 }} />
          ) : (
